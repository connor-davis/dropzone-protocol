...
class ExpressServer {
  ...
  async listen() {
    ...
    this.httpServer.listen(this.port, async () => {
      console.log(`> Http listening on http://localhost:${this.port}`);
...
class SocketServer {
  constructor(options = {}) {
     ...
     * options or default port 8080
     */
    this.port = options.socketPort || process.env.socketPort || 8081;
  }

  onConnect(callback) {
    setTimeout(
      () => this.socketServer.on('connection', (socket) => callback(socket)),
      100
    );
  }
  ...
  async listen() {
     ...
     */
    this.httpServer.listen(this.port, async () => {
      console.log(`> Socket http listening on http://localhost:${this.port}`);
...
module.exports = { ExpressServer, SocketServer };
